...
- **FR-003**: The capture box MUST appear with its text input already focused and ready to receive input, requiring no click or additional action from the user to begin typing, within 100ms of the trigger (hotkey press or app open).
- **FR-003a**: If the global hotkey is pressed while a capture box is already open with in-progress input, the system MUST ignore the second trigger, leaving the open box and its unsaved input untouched.
