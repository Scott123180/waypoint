...
- [X] T071 **Done 2026-08-10 — Principle III proven on Apple Silicon.** The user installed the CI-built v0.1.0 artifact on the MacBook and dictated successfully **with Wi-Fi off**, so the bundled `ggml-small.en.bin` needs no network and voice capture is genuinely offline on macOS. This was the last claim in the project resting on a machine nobody had run. Two caveats recorded rather than glossed: (a) v0.1.0 shipped unsigned and would not launch until the user ran `codesign --sign -` by hand — fixed automatically in v0.1.1 by `scripts/after-pack.js`; (b) quickstart §1, §9, and §11 were not separately re-run on macOS, though §5 is the one that carried the offline risk. Original text: Download the CI-built macOS artifact on the MacBook and re-run quickstart scenarios 1, 5, 9, and 11 with the network off to prove the bundled model needs no download (Principle III)
...
- [X] T078 **Done 2026-08-10.** v0.1.0 and v0.1.1 tagged and published; the workflow builds arm64 DMG, macOS zip, and Linux AppImage. Getting there took six packaging defects on a path that had never been executed, plus an unsigned-bundle failure that made the macOS app unlaunchable — all recorded in the git history. Offline validation on the MacBook is covered under T071.
