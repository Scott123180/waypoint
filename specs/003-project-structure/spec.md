...
- **Completion dates are calendar dates (`YYYY-MM-DD`) in the user's local timezone**, matching how `waiting.md`, `calendar.md`, and `trash.md` already record dates. Date-level granularity is enough to review completed work over a range, and a wall-clock date is what the user means by "the day I finished it."
...
- Explicitly out of scope and deferred: the top-three / WIP limit (Feature 4), the weekly review ritual (Feature 5), the local HTTP/JSON API (Feature 6), and any AI-assisted structuring, splitting, or suggestion (Feature 7) — including automatic conversion of an unprocessed item into a milestone or next action (FR-046c).
