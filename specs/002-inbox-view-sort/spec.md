...
- **Calendar-flagged items go to a dedicated append-only `calendar.md`**, parallel to `waiting.md`. No calendar file exists in the roadmap's data model yet, so this feature establishes one as a staging list for whichever later feature does the integration. It deliberately mirrors waiting-for's shape — text plus a date that starts a clock — because "flagged but never scheduled" is the same staleness problem as "delegated but never returned", and a later feature should be able to check both the same way.
...
- **The discard list is a new durable artifact** not present in the roadmap's data model, added by this feature as the soft-delete target. It grows without bound; pruning it is a deliberate non-goal here rather than an oversight, and a later feature may add retention if the file ever becomes a nuisance.
