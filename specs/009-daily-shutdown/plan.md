...
*Recorded 2026-08-19, on completing the tasks. Four things the design did not
anticipate, and one prediction that held exactly as written.*

**The `projects/` failure naming concession held as predicted.** `ProjectService.readAll`
propagates the error, the panel is named `projects/` rather than by file, and the other three panels stay
built and actionable. No shipped read loop was touched, and
`shutdown-source-failure.test.ts` asserts both halves.

| Deviation | What was planned | What shipped, and why |
|---|---|---|
| **No undo affordance on the capture box** | T088 said "render the same undo affordance the capture surface renders after a successful capture", and `contracts/shutdown-api.md` §4 listed `undoCapture` on the bridge | The capture surface renders **none**. Feature 1 scoped undo to *dictated* captures — `CaptureService.submit` opens an undo window only when `source === "dictated"` (001 FR-009, FR-018) — and the control lives in the tray, not in any window. A typed capture from this screen is undoable exactly as a typed capture from the box is: it opens no window and the tray entry stays disabled. That inheritance *is* FR-049. Building a button would have given the shutdown an affordance no other surface has (Principle VII), so `undoCapture` was left off the bridge too — an unused bridge method is surface a client could come to depend on. `shutdown-capture-undo.test.ts` asserts the symmetry directly, in both directions |
| **`policyNotices` is read from `policy.md`, not harvested from a decision** | data-model.md named `parsePolicyConfig(..., { withProblems: true })` but did not say where it would be called | It is called in `ShutdownService.readPolicyNotices()`, and **only `problems` is destructured**. FR-030 wants the notice whether or not a rule fires, and this screen asks the rule once per candidate — a vault with nothing stale would otherwise never mention a typo the user needs to fix. The review surfaces the same complaint from its unconditional inbox step for the same reason. The Principle V boundary is asserted rather than assumed: `shutdown-scope-boundaries.test.ts` fails if the service ever names `stalenessDays`, compares anything to a `config.` value, or decides what stale means |
| **`ProjectSource` is exported as `ShutdownProjectSource`** | `contracts/shutdown-api.md` §1 listed `export type { ... ProjectSource ... }` | `RetrospectiveService` already exports a `ProjectSource` from `packages/core/src/index.ts`, and the two shapes differ — the retrospective's carries `project` alone, this one carries `project` and `summary`. Re-exporting the name would have been a collision; aliasing keeps both reachable and makes the difference visible at the call site |
| **`ShutdownService` reads five sources, not four** | The plan says "four reads build the whole screen" | Still true of the *panels*: `top-three.md`, `identity.md` + one read per project file, `waiting.md`, `calendar.md`, each exactly once. `policy.md` is the fifth path, read once for the notices above and once per candidate by `DefaultPolicy.decide()` — already excluded from the count by FR-011a and SC-013 for the second reason, now also for the first. `shutdown-reads.test.ts` asserts the panel sources are read once each **and** that `policy.md` is read more often than there are candidates, so the exclusion is visible rather than assumed |
