...
- [X] T106 Run `npm run typecheck && npm test` clean using the scripts in `package.json`, then `npm run dev` for a final manual pass
  ...
  - **Amended 2026-08-15, after convergence:** the manual pass was performed by the user and succeeded, closing this out via T111. Final automated figures at that point were 1487 unit tests and 168 Playwright specs.
...
**Outcome, recorded 2026-08-15.** All six landed. Five were the agent's; T111 was the user's, and was closed
after v0.5.0 shipped by running `npm run dev`. Two things worth
...
- [X] T111 Perform the `npm run dev` manual pass per T106 (partial), or record it explicitly as an unmet release gate in this file. The remaining GUI-only checks after T110 lands are quickstart §4's two-subject staleness in the running app and any visual judgement the specs cannot make. This task is a person's to close — do not mark it `[X]` on the strength of the automated suite
  - **Closed by the user on 2026-08-15**, after v0.5.0 was cut: `npm run dev` was run and succeeded. This is the one task in the feature no agent could close, which is why it was left open through the release rather than absorbed into the automated suite's green result.
